...
\item \textbf{stringa su A}: sequenza finita di simboli di un alfabeto A
\item \textbf{lunghezza di una stringa}: numero di simboli che la compongono
\item \textbf{concatenazione}: operazione che consiste nel mettere due stringhe in sequenza
\item \textbf{linguaggio sulla'alfabeto A}: insieme di stringhe su A
\end{itemize}

date due stringhe s e t su A, la loro concatenazione e indicata con st

ora possiamo definire alcune \underline{operazioni tra linguaggi sullo stesso alfabeto}
\begin{itemize}
\item \textbf{concatenazione}: dato un limguaggio L e un linguaggio M, la loro concatenazione si dice LM o L·M ed e l'insieme di tutte le stringhe ottenute concatenando una stringa di L con una stringa di M. se uno dei due e un singoletto(insieme di 1 elemento),l'elemento puo rappresentare l'insieme.
\item \textbf{unione}: dato un linguaggio L e un linguaggio M, la loro unione si dice L U M ed e l'insieme di tutte le stringhe che appartengono a L o a M.
\item \textbf{chiusura di Kleene}: dato un linguaggio L, \underline{la sua chiusura di Kleene si dice L*} ed e l'insieme di tutte le stringhe ottenute concatenando un numero qualsiasi di stringhe di L, compresa la stringa vuota. in pratica e l'unione di $L^0,L^1,L^2$ ecc..
\item \textbf{chiusura positiva di Kleene}: dato un linguaggio L, la sua chiusura positiva di Kleene si dice L+ ed e l'insieme di tutte le stringhe ottenute concatenando una o piu stringhe di L. in pratica e l'unione di $L^1,L^2$
\end{itemize}

\subsubsection{espressioni regolari}

Un’\textbf{espressione regolare} descrive un insieme di stringhe.
Questo insieme è un linguaggio ben definito e contiene tutti i possibili lessemi
che corrispondono a un certo token.

i token sono classi di lessemi(identifier,number ecc...) i lessemi sono i simboli concreti.

dato un linguaggio L e un'espressioone regolare R, esiste un solo linguaggioL(R), cioe un solo insieme di tutte le stringhe che R descrive.

quindi in pratica un'espressione regolare e l'insieme di tutti i lessemi che corrispondono a un certo token(ad esempio se il token e identificatore x,y ecc... appartengono all'esspressione regolare)

tra due espressioni regolari sullo stesso linguaggio ci sono diverse proprieta: es commutativa, associativa ecc..

\subsubsection{definizioni regolari}
Per scrivere espressioni complesse è utile poterle suddividere in sottoespressioni, ognuna definita separatamente con un nome associato.
Fissato un alfabeto A, una definizione regolare su A è costituita da una lista di
definizioni
...
nella quale tutti 1 dn sono simboli nuovi, distinti tra loro e non appartenenti ad A,
mentre ogni sn è un'espressione regolare sull'alfabeto A U { d1  ...., di- 1}.

\subsection{automi finiti}

le espressioni regolari specificano la forma deilessemi associati ad un token, ma rimane il problema di identificarli prima e associarli.

un \textbf{automa finito}: è un algoritmo con un pool di stati, che prende in input un lessema, consuma uno alla volta i caratteri del lessema e cambia stato in base al carattere letto e allo stato corrente.
dopo aver consumato l'ultimo carattare, se l'automa si trova in uno stato di accettazione(tali stati sono detti stati finali), il lessema e riconosciuto come appartenente al token associato all'automa. altrimenti se non si trova in uno stato di accettazione o si e fermato prima rifiuta l'input.

\subsubsection{automi finiti non deterministici}

in certi casi puo essere utile avere piu scelte su quale stato andare in base al carattere letto e allo stato corrente.
in questo caso si parla di \textbf{automa finito non deterministico}: ogni coppia (stato corrente,carattere letto) puo portare a piu stati successivi.
in questo caso l'automa accetta il lessema se esiste almeno un percorso che porta a uno stato di accettazione.

\textbf{definizione formale}: un automa finito non deterministico è una quintupla(S,Q,d,q0,F) dove 

\begin{itemize}
\item S è l'alfabeto degli input
\item Q è l'insieme finito degli stati
\item d è la funzione di transizione che associa ad ogni coppia (stato corrente, carattere letto) un insieme di stati successivi.
\item q0 è lo stato iniziale
\item F è l'insieme degli stati finali
\end{itemize}

\subsubsection{automi finiti deterministici}
un DFA(definite finite automata) e un automa finito non deterministico in cui ogni coppia (stato corrente,carattere letto) porta a un solo stato successivo.
\begin{itemize}
\item una sola computazione possibile
\item piu semplice da implementare
\item nessuna ambiguita
\item efficente
\end{itemize}

a partire da qulasiasi NFA(non deterministic finite automata) e possibile costruire un DFA equivalente che accetta lo stesso linguaggio e viceversa.

in un NFA ci sono 2 cause di non determinismo:
\begin{itemize}
\item   piu stati successivi per la stessa coppia (stato corrente,carattere letto)
\item transizioni epsilon, che permettono di cambiare stato senza leggere alcun carattere
\end{itemize}       

di solito e buona norma trasformare gli NFA in DFA per semplificare l'implementazione.

cio si fa attraverso la \textbf{costruzione per sottoinsiemi}: cioe ogni stato del DFA corrisponde a un sottoinsieme di stati dell'NFA. Uno stato dell'DFA e finale se contiene almeno uno stato finale dell'NFA.

\subsection{da espressioni regolari ad automi finiti}

ora ci serve un metodo sistematico per costruire un'NFA che riconosce los stesso linguaggio a partire da un'espressione regolare.

data un'espressione regolare s possiamo costruire un'NFA N[s] tale che N[s]= L(N[s])

durante la costruzione devono essere mantenute 2 proprieta:
\begin{itemize}
    \item lo stato iniziale non ha archi entranti
    \item lo stato finale non ha archi uscenti
\end{itemize}

in pratica si costruisce l'NFA e da cio si ricava il DFA

riassumendo:

\underline{Ogni espressione regolare definisce un linguaggio regolare; per essa è possibile}

\underline{costruire un NFA equivalente e, tramite la costruzione per sottoinsiemi, un DFA che riconosce}

\underline{lo stesso linguaggio.}

\subsection{automi finiti e grammatiche}

i linguaggi si poterbbero anche descrivere anche attraverso grammatiche libere da contesto, ma queste di loro sarebbero troppo potenti perche abbia senso

si puo rendere fattibile limitando la forma delle produzioni

ora dimostreremo che ogni DFA corrisponde a una certa grammatica libere da contesto
per poi dimostrare che ogni grammatica regolare definisce un linguaggio
corrispondente ad un'espressione regolare.
vedi  pag 65(82)

\subsection{minimizzare un DFA}


\section{(secondo semestre) ambiente e scope}

nome= sequenza di caratteri usata per denotare qualcos'altro.

un oggetto e denotabile quando gli puo essere associato un nome

binding o legame: associazione tra nome e oggetto

il tempo di binding puo essere statico o dinamico

Ambiente:
insieme delle associazioni fra nomi e oggetti denotabili
esistenti a run-time in uno specifico punto del programma ed
in uno specifico momento dell’esecuzione

Dichiarazione:
meccanismo (implicito o esplicito) col quale si crea
un’associazione in ambiente

\underline{lo stesso nome puo denotare oggetti distinti in punti diversi del programma}

\textbf{aliasing}: nomi diversi denotano lo stesso oggetto.

nei linguaggi moderni l'ambiente è strutturato in blocchi.
...
fine del legame non necesssariamente coincide con la fine dell'oggetto.

la disattivazione annulla il legame nome-oggetto,un nome smette temporaneamente di riferirsi al suo oggetto
perché è stato mascherato (shadowing) da una nuova dichiarazione.
 la riattivazione fa in modo che il vecchio legame torna visibile
quando esci dal blocco che lo mascherava
...
esistono diversi tipi di allocazione della memoria, ci sono due politiche principali:

\begin{itemize}
\item allocazione statica: la memoria viene allocata al tempo della compilazione. un oggetto ha un indirizzo assoluto che viene mantenuto per tutta l'esecuzione del programma, non supporta ricorsione.
\item allocazione dinamica: la memoria viene allocata al tempo dell'esecuzione 
\end{itemize} 
allocazione con stack: oggetti allocati con politica LIFO
 allocaione in heap: oggetti allocati e deallocati in qulasiai momento



ricorsione con allocazione statica creerebbe troppe istanze della della stessa varibile  locale

con allocazione diinamica, ogni istanza di un sottoprogramma è gestita attraverso il suo \underline{record di attivazione} gestiti grrazie a una pila.

ogni blocco ha un suo record di attivazione

la gestione della pila è gestita attraverso 
\begin{itemize}
\item sequenza di chiamata: codice del chiamante prima della chiamata
\item prologo: codice eseguito all'inizio del blocco
\item epilogo: codice eseguito alla fine
\item sequenza di ritorno codice eseguito dal chimante subito dopo la chiamata
\end{itemize}

i vari RDA(record di attiavzione) sono collegati con un puntatore dinamico, visto che gli rda sono di dimensione diversa gli uni dagli altri e la dmensione dei dati al loro interno pou variare a runtime.\\

l'allocazione dinamica puo avvenire anche con heap oltre che con stack

\textbf{heap}: all'interno dell'heap i blocchi di memoria possono essere allocati e deallocati in momenti arbitrari

l'heap e diviso in blochi di dimensione fissa, e in rigine sono tutti collegati in una lista libera.

avviene frammentazione  e serve un'attenta gestione della memoria

la lista libera inizialmente occupa un solo blocco della dimesione dell'heap

quando il blocco e deallocato, viene restituito alla lista libera.

ad ogni richiesta di allocazione si cerca un blocco di dimensione opportuna, lo si puo cercare con \underline{first fit}(il primo che trova di dimensione minima) o \underline{best fit}: il piu piccolo di dimensione minima.
