...
\underline{processo del client}: processo che inizializza la comunicazione

\underline{processo del server}: processo che aspetta di essere contattato


i \textbf{socket}: porta da cui partono e arrivano messaggi per gli altri processi


per ricevere messaggi i processi devono avere degli identificatori.

l'identificatore contiene sia l'indirizzo IP che il port number associato con il processo.


app diverse possono richiedere cose leggermente diverse nel trasporto dati, ad esempio alcune tollerano qualche perdita di dati, altre no.


ci sono due principali protocolli di trasporto dati \textbf{TCP} e \textbf{UDP}, vedi scorso capitolo.


\textbf{SSL}: provvede crittatura TCP e integrita dei dati.


\textbf{HTTP}: hypertext transfer protocol, usato sia dal client che dal server per mostrare o mandare oggetti.

HTTP usa TCP.

il client inizializza la connessione TCP con il server-> il server la accetta-> scambio di informazioni attraverso messaggi HTTP->chiusura connessione TCP

HTTP è \underline{stateless} ovvero il server non mantiene informazioni a riguardo delle passate richieste del client.

esistono connessioni HTTP \underline{persistenti} e \underline{non persistenti.}
...
processo di una connessione non persistente: client inizializza la connessione TCP-> server accetta la connessione nel port-> client manda un message request HTTP nel socket di connessione TCP- il serevr lo riceve e manda un response message->il server ciude la connessione TCP->il client riceve la risposta contenente il file html, poi eventualmenet ripete i passi precedenti per ogni oggetto referenziato.

definizioni: \textbf{RTT}: tempo che un piccolo pacchetto  ci mette a viaggiare dal client al server e tornare

textbf{HTTP response time}: 2RTT+ tempo di trasmissione del file.


le connessioni non persistenti richiedono 2 RTT mentre quella ppersistente ce ne mette 1 per tutti gli oggetti.

abbiamo quindi sue tipi di messaggi HTTP: \textbf{request} e \textbf{response}

per fare uploading esiste il metodo post e il metodo URL.

nella prima linea della risposta server-client appare lo \underline{status code}

\begin{itemize}
\item 200: ok, oggetto richiesto piu avanti nel messaggio
\item 301: oggetto spostato permanentemente,nuova locazione piu avanti nel messaggio
\item 400: bad request, messaggio di richiesta non capito dal server
\item 404: not found, oggetto richiesto non trovato nel server
\item 505: versione di HTTP non supportata.
\end{itemize}
...
4 componenti:
\begin{itemize}
\item linea header cookie del messaggio di risposta
\item linea header cookie del prossimo messsaggio di risposta
\item il file cookie tenuto nell'host dell'user, gestito dal browser dell'user
\item il database di back-end del sito.
\end{itemize}


\underline{server proxy o web caches}
...
l'SMTP usa TCP per mandare i messsaggi in sicurezza
...
SMTP usa connessioni persistenti e richiede il messaggio di essere im ascii a 7 bit.

\underline{RFC 822}: standard sul format del testo del messaggio

user agent-> server della mail del mittente-> server della mail del ricevente-> mail access protocol-> user agent

il \underline{mail access protocol} serve a recuperare messaggi dalla mail del ricevente

ci sono diversi protocolli: POP, IMAP(piu features), HTTP(gmail,hotmail ecc.....)


POP3 ad esempio e diviso in fase di autorizzazione e fase di transazione in cui recupera il messaggio e poi cancella, quindi il messaggio non puo piu essere letto su altri client.
POP3 inoltre e stateless
...
\textbf{DNS}: domain name system, traduce nomi degli indirizzi web in indirizzi IP comprensibili dal computer.

è composto da una gerarchia di diversi \underline{name servers} che collaborano tra loro per risolvere i nomi

root name server è l'origine della gerarchia, contattato da name server locale che non riesce a risolvere il nome.

esistono i \underline{top level domains(TLD)},resposabili per com,it,net ecc...
...
i server DNS possono mettere in cache una mappatura, ma cio puo anche andare out-of-date

le informazioni salvate si dicono: \underline{resource record}
...
di solito si usano attacchi DDoS, cioe si bombarda il server di traffico, non molto pericoloso.

e piu pericoloso attaccare i TLD


si puo anche intercettare le queries con man-in-the-middle

o usare DNS poisoning, cioe replies dannose ai server DNS


\subsection{P2P}

i server non sono sempre on,i peers sono connessi a intermittenza e cambiano ip address

con meccanismo client-server il server maanda copie dei file a intermittenza e ogni client li scarica

con p2p il server deve mandare almeno 1 copia e ogni client la scarica, molteplici client scaricano e si scambiano grosse quantita di bits e formano un \underline{bit torrent}

peer differenti possono avere raggrupamenti diversi di dati, poi se li scambiano a richiesta

saltata parte video

\subsection{socket programming}

serve a costruire applicazioni client e server che comunicano usando socket

UDP e TCP sono due tipi di socket, come gia detto TCP e piu affidabile

UDP non ha handshake e non ce una vera e propria connessione ma piuttosto il mittente attacca indirizzo ip e porta di destinazione a ogni pacchetto

i dati potrebbero andare persi o arrivare sparsi


con TCP invece il server crea una socket di ascolto che accoglie i client che cervano una connessione, il client crea una socket per il server, 3 way handshake  epoi il server crea una socket specifica per quel client, su cui poi comunicheranno


\subsection{NAT}

NAT (Network Address Translation), una tecnica usata praticamente in tutti i router domestici per permettere a più dispositivi di condividere un unico indirizzo IP pubblico.


ogni dispositivo ha il suo indirizzo della rete privata, ma internet non riuscirebbe a leggerlo, normalmente servirebbe un ip pubblico per ogni dispositivo.

invece con NAT viene usato il solo ip publlico el router, permettendo cosi di cambiare indirizzi privati, ISP e ad avere maggiore sicurezza.

perche il router distingua i dispositivi enntra in gioco TCP/UDP, \underline{una connessione e identificata da indirizzo ip+ porta}, quindi il router, la prima volta che riceve un messaggio per quel dispositivo traduce questa coppia in un numero e lo salva in una tabella, la tabella NAT


c'è qualche difetto, cioe il fatto che crea problemi con alcune applicazioni P2P, i router vanno a modificare cosi le porte, e che la connessione non è piu end-to-end, diminuendo la sicurezza, inoltre con IP6 si poterbbero avere sul serio indirizzi IP per tutto.
